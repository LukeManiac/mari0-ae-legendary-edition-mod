...
function mario:init(x, y, i, animation, size, t, properties)
	...
	if properties and (not editormode) then
		...
		if properties.helmet then
			self:helmeted(properties.helmet, true)
		end
				...
				for i, v in pairs(enemies) do
					if objects[v] then
						for j, w in pairs(objects[v]) do
							if w.active and w.autodelete and onscreenobj(w) then
	...
	elseif self.animation == "doorexit" then
		...
		if self.animationtimer > 0.3 then
			for i, v in pairs(enemies) do
				if objects[v] then
					for j, w in pairs(objects[v]) do
						if w.horautodeleteimmunity and onscreenobj(w) then
...
function mario:shoed(shoe, initial, drop) --get in shoe (type, make sound?, drop shoe?)
	soundfordrop = helmetremovesound

	if shoe then
		canplayshoesound = true
		self.shoeoffsetY = 0
		if self.ducking then
			self:duck(false)
		end
		...
		if self.shoe then
			...
			self.shoe = shoe
			canplayshoesound = not initial
		end

		soundforshoe = swimsound
		...
		elseif shoe == "drybonesshell" then
			...
			soundforshoe = drybonesshellsound
		end

		if canplayshoesound then
			playsound(soundforshoe)
		end
...
function mario:helmeted(helmet, initial)
	if helmet then
		if self.helmet or self.size == -1 or (helmet == "cannonbox" and (self.size == 8 or self.size == 16)) then
			...
			if not initial then
				if helmet == "beetle" then
					playsound(helmetsound)
				elseif helmet == "spikey" then
					playsound(helmetspikesound)
				end
			end
