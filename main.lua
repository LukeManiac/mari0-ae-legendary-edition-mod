--[[
	Majority of the content is from Maurice Guégan. If not, it's pretty obvious.
	Credits to him. No stealing!
	...
	This license only applies to the code and original other assets, as it should.
	Anyways, enjoy.
...
--[[
	-----------------------------------------------------------------------------
	"LEGENDARY" mod by LukeManiac (that would be me)
...
function love.load()
	...
	intro_image = love.graphics.newImage("graphics/intro.png")
	...
	--intro
	loadingtexts = {"nah jit trippin", "williamfrog is a legendary creator", "go check out m0rtal engine!", "sonion im crine", "oewertom be making anything these days",
					"you look like a ticklish neck", "###############", "wondersick be making literally anything", "linganguliguliguli wacha lingangu lingangu",
					"diamonds! that's la peace", "nothin' beats a jet2holiday", "homer let the barts out", "cityboiiiiii", "food don't sing but chicken wraps", "it's-a me mario",
					"fairs", "faahhhhhhh", "i just hit the jackpot hey hey hey hey", "what da dog doin", "it's the one and only do double g", "did he just say that his last name is burger",
					"must've been the wind", "chicken butt", "kaboom kablow", "why? why? tell em that it's human nature", "stop taking everything down pls nintendo",
					"he took a bite of a boneless wing", "helo pigeoon e e e", "if you're homeless, buy a house", "if you're in a coma, wake up", "if you're disabled, enable yourself",
	...
	--AUDIO--
	--sounds
	jumpsound = newsound("jump")
	jumpbigsound = newsound("jumpbig")
	jumptinysound = newsound("jumptiny")
	stompsound = newsound("stomp")
	shotsound = newsound("shot")
	errorsound = newsound("error")
	blockhitsound = newsound("blockhit")
	blockbreaksound = newsound("blockbreak")
	coinsound = newsound("coin")
	pipesound = newsound("pipe")
	boomsound = newsound("boom")
	mushroomappearsound = newsound("mushroomappear")
	mushroomeatsound = newsound("mushroomeat")
	shrinksound = newsound("shrink")
	deathsound = newsound("death")
	gameoversound = newsound("gameover")
	fireballsound = newsound("fireball")
	meltsound = newsound("melt")
	freezesound = newsound("freeze")
	iceballsound = newsound("iceball")
	iceballhitsound = newsound("iceballhit")
	iceblockbreaksound = newsound("iceblockbreak")
	oneupsound = newsound("oneup")
	levelendsound = newsound("levelend")
	castleendsound = newsound("castleend")
	scoreringsound = newsound("scorering", "static", true)
	intermissionsound = newsound("intermission", "stream")
	firesound = newsound("fire")
	bridgebreaksound = newsound("bridgebreak")
	bowserfallsound = newsound("bowserfall")
	vinesound = newsound("vine")
	swimsound = newsound("swim")
	rainboomsound = newsound("rainboom")
	konamisound = newsound("konami")
	pausesound = newsound("pause")
	...
	raccoonswingsound = newsound("raccoonswing")
	raccoonplanesound = newsound("raccoonplane", "static", true)
	skidsound = newsound("skid")
	turretshotsound = newsound("turretshot")
	pbuttonsound = newsound("pbutton", "stream")
	windsound = newsound("wind", "static", true)
	suitsound = newsound("suit")
	magicsound = newsound("magic")
	koopalingendsound = newsound("koopalingend")
	bowserendsound = newsound("bowserend")
	dooropensound = newsound("dooropen")
	doorclosesound = newsound("doorclose")
	keysound = newsound("key")
	keyopensound = newsound("keyopen")
	heelsound = newsound("heel")
	weirdmushroomsound = newsound("weirdmushroom")
	jumpskinnysound = newsound("jumpskinny")
	energybouncesound = newsound("energybounce")
	shufflesound = newsound("shuffle", "static", true)
	grabsound = newsound("grab")
	throwsound = newsound("throw")
	switchsound = newsound("switch")
	iciclefallsound = newsound("iciclefall")
	helmetsound = newsound("helmet")
	helmetspikesound = newsound("helmetspike")
	helmethitsound = newsound("helmethit")
	helmetremovesound = newsound("helmetremove")
	drybonesshellsound = newsound("drybonesshell")
	propellersound = newsound("propeller")
	clearpipesound = newsound("clearpipe")
	checkpointsound = newsound("checkpoint")
	megamushroomsound = newsound("megamushroom", "stream")
	powblocksound = newsound("powblock")
	cannonfastsound = newsound("cannonfast")
	bumperhitsound = newsound("bumperhit")
	bumperjumpsound = newsound("bumperjump")
	stompbigsound = newsound("stompbig")
	thwompbigsound = newsound("thwompbig")
	superballsound = newsound("superball")
	superballeatsound = newsound("superballeat")
	capeflysound = newsound("capefly")
	mushroombigeatsound = newsound("mushroombigeat")
	groundpoundsound = newsound("groundpound")
	sausagesound = newsound("totaka")--this is for um, the sausage entity.
	--i don't like this but i KNOW someone is going to complain about sounds if i don't do this
	collectable1sound = newsound("collectable1")
	collectable2sound = newsound("collectable2")
	collectable3sound = newsound("collectable3")
	collectable4sound = newsound("collectable4")
	collectable5sound = newsound("collectable5")
	collectable6sound = newsound("collectable6")
	collectable7sound = newsound("collectable7")
	collectable8sound = newsound("collectable8")
	collectable9sound = newsound("collectable9")
	collectable10sound = newsound("collectable10")

	glados1sound = newsound("glados1", "stream")
	glados2sound = newsound("glados2")

	portal1opensound = newsound("portal1open", "static", nil, 0.3)
	portal2opensound = newsound("portal2open", "static", nil, 0.3)
	portalentersound = newsound("portalenter", "static", nil, 0.3)
	portalfizzlesound = newsound("portalfizzle", "static", nil, 0.3)

	lowtimesound = newsound("lowtime")
	...
	--mouse cursors
	if (not android) and (not androidsafe) then  --love2d android is dogwater, windows better
...
function mini_if(condition, trueval, falseval)
	...
	return condition and trueval or falseval
end

function rainbow(timer, hue_speed, saturation, value)
    timer = timer or 0
    hue_speed = hue_speed or 1
    saturation = saturation or 1
    value = value or 1

    local h = (timer * hue_speed) % 1
...
function properlerp(start, stop, step)
...
local motion_curves = {
    ...
    back = function(t)
        return ((1.70158 + 1) * t ^ 3) - (1.70158 * t ^ 2)
    end,
    ...
    soft_snap = function(t)
        return t - math.sin(2 * math.pi * t) / (2 * math.pi)
    end,

    hesitate = function(t)
        if t < 0.5 then
            return 0.18 * math.sin(2 * math.pi * t)
        else
            return 0.18 + 0.82 * ((t - 0.5) / 0.5) ^ 0.65
        end
    end,

    slingshot = function(t)
        if t < 0.35 then
            return -0.12 * math.sin((t / 0.35) * math.pi)
        else
            return -0.12 * (1 - (t - 0.35) / 0.65)
                + 1.12 * ((t - 0.35) / 0.65) ^ 0.55
        end
    end,

    glide_wave = function(t)
        return t + 0.08 * math.sin(math.pi * t) * math.sin(3 * math.pi * t)
    end,

    drift_pull = function(t)
        return t + 0.22 * t * (1 - t) * math.sin(2.5 * math.pi * t)
    end,

    ripple_push = function(t)
        return t + 0.12 * (1 - t)
            * math.sin(5 * math.pi * t)
            * math.sin(math.pi * t / 2)
    end,

    arch = function(t)
        return t + 0.9 * t * (1 - t) * (0.5 - t)
    end,

    spring_ramp = function(t)
        return 1 - (1 - t)
            * (1 - 0.18 * math.sin(7 * math.pi * t) * (1 - t))
    end,

    magnet = function(t)
        if t < 0.75 then
            return 0.78 * (1 - math.cos((t / 0.75) * math.pi / 2))
        else
            return 0.78 + 0.22
                * math.sin(((t - 0.75) / 0.25) * math.pi / 2)
        end
    end,

    tidal = function(t)
        return t + 0.14 * math.sin(math.pi * t) * (1 - 2 * t) ^ 2
    end,

    dip = function(t)
        return t - 0.16 * math.sin(math.pi * t) * (1 - t)
    end,

    crest = function(t)
        return t + 0.16 * math.sin(math.pi * t) * t
    end,

    micro_bounce = function(t)
        return t + 0.06
            * math.sin(10 * math.pi * t)
            * (1 - t) ^ 1.5
    end,

    brake_release = function(t)
        if t < 0.4 then
            return 0.55 * (1 - math.cos((t / 0.4) * math.pi / 2))
        else
            return 0.55 + 0.45 * (
                ((t - 0.4) / 0.6)
                + 0.08 * math.sin(2 * math.pi * (t - 0.4) / 0.6)
            )
        end
    end,

    float_fall = function(t)
        return 1
            - math.cos((t ^ 0.7) * math.pi / 2)
            * (1 - 0.08 * math.sin(4 * math.pi * t))
    end,

    hook = function(t)
        return t + 0.25 * (t - t * t) * (0.3 - t)
    end,

    echo = function(t)
        return t + 0.1 * math.sin(6 * math.pi * t) / (1 + 12 * t)
    end,

    whiplash = function(t)
        if t < 0.2 then
            return -0.08 * math.sin((t / 0.2) * math.pi)
        else
            return -0.08 * (1 - (t - 0.2) / 0.8)
                + 1.08 * math.sin(((t - 0.2) / 0.8) * math.pi / 2)
        end
    end,
    ...
    settle = function(t)
        return 1
            - (1 - t) * (1 - t)
            * (1 + 0.35 * math.sin(3 * math.pi * t) * (1 - t))
    end
}

function motion_curve(t, curve, curve_in, curve_out, clamp_input, clamp_output)
    curve_in = curve_in or false
    curve_out = curve_out or false
    clamp_input = clamp_input or false
    ...
    if curve_in and curve_out then
        if t < 0.5 then
            out = get(t * 2) / 2
        else
            out = (1 + (1 - get(2 * (1 - t)))) / 2
...
function get_corner_radius(width, height)
...
function loadnitpicks()
	...
	if love.filesystem.getInfo("alesans_entities/nitpicks.json") then
		...
		DPIFix = t.dpifix
		NoEditorPreviews = t.noeditorpreviews
	end
end
...
function newsound(soundname, soundtype, soundloop, soundvolume)
    local sound = love.audio.newSource("sounds/" .. soundname .. ".ogg", soundtype or "static")

	if soundloop or (soundloop == nil and soundtype == "stream") then
