...
function game_load(suspended, deletesuspend)
	...
	pausemenuopen = false
	pausemenuopentimer = 0
	pausemenuopenduration = 0.5
	menuprompt = false
	menuprompttimer = 0
	menupromptduration = 0.5
	desktopprompt = false
	desktopprompttimer = 0
	desktoppromptduration = 0.5
	suspendprompt = false
	suspendprompttimer = 0
	suspendpromptduration = 0.5
...
function game_draw()
	for split = 1, #splitscreen do
		...
		if showplayercoords then
			for i = 1, players do
				...
				if v.drawable then
					centerprint("x: " .. truncatedecimal(v.x, 3), (v.x-xscroll)*16, (v.y-yscroll-2)*16)
					centerprint("y: " .. truncatedecimal(v.y, 3), (v.x-xscroll)*16, (v.y-yscroll-1.375)*16)
				end
			end
		end
	...
	love.graphics.translate(0, yoffset*scale)
	local winwidth, winheight = getproperwindowsize()

	if testlevel then
		love.graphics.setColor(255, 255, 255, 255)
		centerprint("TESTING COURSE - PRESS ESC TO RETURN", winwidth/scale/2, (winheight*2-10)/scale/2)
	end
	
	--pause menu
	if pausemenuopentimer > 0 then
		bettertimer = motion_curve(pausemenuopentimer / pausemenuopenduration, "circ", pausemenuopen, not pausemenuopen, true)
		pausewidth = winwidth*0.75
		pauseheight = winheight*0.75
		pauseradius = get_corner_radius(pausewidth, pauseheight) / 2
		prevlinewidth = love.graphics.getLineWidth()
		love.graphics.setScissor((winwidth - (pausewidth * bettertimer)) / 2, (winheight - (pauseheight * bettertimer)) / 2, pausewidth * bettertimer, pauseheight * bettertimer)
		love.graphics.setLineWidth(1)
		love.graphics.setColor(0, 0, 0)
		love.graphics.rectangle("fill", (width*8*scale)-150*scale, winheight/8, pausewidth, pauseheight, pauseradius)
		love.graphics.setColor(255, 255, 255)
		love.graphics.rectangle("line", (width*8*scale)-150*scale, winheight/8, pausewidth, pauseheight, pauseradius)
		love.graphics.setLineWidth(prevlinewidth)
		
		for i = 1, #pausemenuoptions do
			love.graphics.setColor(100, 100, 100, 255)
			if pausemenuselected == i and not (menuprompt or desktopprompt) then
				love.graphics.setColor(255, 255, 255, 255)
				properprint(">", (width*8*scale)-145*scale, (112*scale)-60*scale+(i-1)*25*scale)
			end
			if dcplaying and pausemenuoptions[i] == "save game" then --restart instead of suspend
				properprintF(TEXT["restart"], (width*8*scale)-135*scale, (112*scale)-60*scale+(i-1)*25*scale)
			elseif alwaysdeletesuspend and pausemenuoptions[i] == "save game" then
				properprintF(TEXT["suspend"], (width*8*scale)-135*scale, (112*scale)-60*scale+(i-1)*25*scale)
			else
				properprintF(TEXT[pausemenuoptions[i]], (width*8*scale)-135*scale, (112*scale)-60*scale+(i-1)*25*scale)
			end
			properprintF(TEXT[pausemenuoptions2[i]], (width*8*scale)-135*scale, (112*scale)-50*scale+(i-1)*25*scale)
			
			if pausemenuoptions[i] == "volume" then
				drawrectangle((width*8)-134, 68+(i-1)*25, 74, 1)
				drawrectangle((width*8)-134, 65+(i-1)*25, 1, 7)
				drawrectangle((width*8)-60, 65+(i-1)*25, 1, 7)
				love.graphics.draw(volumesliderimg, math.floor(((width*8)-135+74*volume)*scale), (112*scale)-47*scale+(i-1)*25*scale, 0, scale, scale)
			end
		end

		love.graphics.setColor(100, 100, 100, 255)
		if pausemenuselected == #pausemenuoptions + 1 then
			love.graphics.setColor(255, 255, 255, 255)
			properprint(">", (width*8*scale)-5*scale, (112*scale)-20*scale)
		end

		properprintF(TEXT["shader1:"], (width*8*scale)+5*scale, (112*scale)-20*scale)

		if shaderssupported == false then
			properprintF(TEXT["unsupported"], (width*8*scale)+5*scale, (112*scale)-10*scale)
		else
			properprintF(string.lower(shaderlist[currentshaderi1]), (width*8*scale)+5*scale, (112*scale)-10*scale)
		end

		love.graphics.setColor(100, 100, 100, 255)
		if pausemenuselected == #pausemenuoptions + 2 then
			love.graphics.setColor(255, 255, 255, 255)
			properprint(">", (width*8*scale)-5*scale, (112*scale)+10*scale)
		end

		properprintF(TEXT["shader2:"], (width*8*scale)+5*scale, (112*scale)+10*scale)

		if shaderssupported == false then
			properprintF(TEXT["unsupported"], (width*8*scale)+5*scale, (112*scale)+20*scale)
		else
			properprintF(string.lower(shaderlist[currentshaderi2]), (width*8*scale)+5*scale, (112*scale)+20*scale)
		end

		love.graphics.setScissor()

		if menuprompttimer > 0 then
			drawprompt(menuprompttimer, menupromptduration, menuprompt)
		...
		if desktopprompttimer > 0 then
			drawprompt(desktopprompttimer, desktoppromptduration, desktopprompt)
		...
		if suspendprompttimer > 0 then
			drawprompt(suspendprompttimer, suspendpromptduration, suspendprompt)
...
function drawprompt(timer, duration, open)
	local winwidth, winheight = getproperwindowsize()
	prompttimer = motion_curve(timer / duration, "circ", open, not open, true)
	promptwidth = winwidth/2
    promptheight = winheight/4
    promptradius = get_corner_radius(promptwidth, promptheight) / 2
    prevlinewidth = love.graphics.getLineWidth()
    love.graphics.setScissor((winwidth - (promptwidth * prompttimer)) / 2, (winheight - (promptheight * prompttimer)) / 2, promptwidth * prompttimer, promptheight * prompttimer)
    love.graphics.setLineWidth(1)
    love.graphics.setColor(0, 0, 0, 255)
    love.graphics.rectangle("fill", (winwidth-promptwidth)/2, (winheight-promptheight)/2, promptwidth, promptheight, promptradius)
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.rectangle("line", (winwidth-promptwidth)/2, (winheight-promptheight)/2, promptwidth, promptheight, promptradius)
    love.graphics.setLineWidth(prevlinewidth)
end
...
function drawplayer(i, x, y, r, pad, drop)
	...
	if draw then
		...
		if not v.graphic then
			print("where the player graphics at?")
			return false
		end
...
function game_keypressed(key, textinput)
	if pausemenuopen then
		if menuprompttimer >= menupromptduration then
			...
			return
		elseif desktopprompttimer >= desktoppromptduration then
			if (key == "left" or key == "a") then
				pausemenuselected2 = 1
			elseif (key == "right" or key == "d") then
				pausemenuselected2 = 2
			...
			return
		elseif suspendprompttimer >= suspendpromptduration then
			if (key == "left" or key == "a") then
				pausemenuselected2 = 1
			elseif (key == "right" or key == "d") then
				pausemenuselected2 = 2
		...
		if pausemenuopentimer >= pausemenuopenduration and menuprompttimer == 0 and desktopprompttimer == 0 and suspendprompttimer == 0 then
			...
			elseif (key == "right" or key == "d") then
				...
				elseif pausemenuselected == #pausemenuoptions + 2 then
	...
	if key == "escape" then
		if not editormode and testlevel then
			stoptestinglevel()
			return
		elseif not (editormode or everyonedead) and (pausemenuopentimer == 0 or pausemenuopentimer >= pausemenuopenduration) then
			pausegame()
		end
	end
...
function pausegame(playpause)
	if playpause == nil then playpause = true end
...
function truncatedecimal(number, decplaces)
	if decplaces == nil then decplaces = 3 end
	factor = math.pow(10, decplaces)
	newnumber = math.floor(number * factor) / factor
	return makewhole(newnumber)
end

function centerprint(text, x, y, scalemult)
	if scalemult == nil then scalemult = 1 end
	local text = tostring(text)
	local textx = (x-(8*scalemult*#(text))/2)*scale
	local texty = (y-(10*scalemult*countlines(text))/2)*scale
	if hudoutline then
		properprintFbackground(text, textx, texty, nil, nil, scalemult)
	else
		properprintF(text, textx, texty, scalemult)
	end
end
...
function getproperwindowsize()
	local winwidth = width*16*scale
	local winheight = height*16*scale
